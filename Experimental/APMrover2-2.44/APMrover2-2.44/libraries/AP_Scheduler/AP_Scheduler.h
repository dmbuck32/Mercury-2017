...
public:
	/*
	The following statement defines a pointer-to-function type. This is one of the areas where 
	there is a significant difference between C, which does not - yet(2010) - require all functions to be
	prototyped before being defined or used; and C++, which does.

	A pointer object, (*) stores the address of another object. A pointer is declared with a leading asterisk. 
	The pointer reads from the asterisk to the left, for example:

	int const *p; reads: point p is a pointer to const int
	int * const p; reads: const point p points to int

	When a function parameter is declared with an array type, the actual type is a pointer, and at runtime the
	address of the first element of the array is passed to the function. The following are the same:
	int sum(int data[], size_t_n);
	int sum(int *data, size_t_n);

	*task_fn_t is a pointer to a function that takes zero paramters and returns void. A declaration of an object
	with a function pointer can be hard to read, so typically you declare the type separately with a the typedef
	declaration, and then declare the object using the typedef name.
	*/
