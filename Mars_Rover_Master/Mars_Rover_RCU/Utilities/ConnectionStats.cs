...
namespace Mars_Rover_RCU.Utilities
{
    public partial class ConnectionStats
    {
        ...
        {
            _mavlink = comPort;
            connectionStatsLoad = new Thread(ConnectionStats_Load);
            connectionStatsLoad.Start();
        }

        void ConnectionStats_Load()
        {
            ...
            var subscriptions = new List<IDisposable>
                                    {
                                        ...
                                        _mavlink.WhenPacketReceived
                                            ...
                                            .Subscribe(x => Console.WriteLine(x.ToString("0"))),
                                        ...
                                        CombineWithDefault(_mavlink.WhenPacketReceived, _mavlink.WhenPacketLost, Tuple.Create)
                                            ...
                                            .ObserveOn(SynchronizationContext.Current)
                                            .Subscribe(x => Console.WriteLine(x.ToString("00%"))),
...
//                                            .Subscribe(ts => this.txt_TimeConnected.Text = ts.ToString()),
    ...
    {
        public static IDisposable SubscribeForTextUpdates<T>(this IObservable<T> source)
        {
            return source
                .ObserveOn(SynchronizationContext.Current).Subscribe(x => Console.WriteLine(x));
        }
    }
